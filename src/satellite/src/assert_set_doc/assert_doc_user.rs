...
pub fn assert_doc_user(context: &AssertSetDocContext) -> Result<(), String> {
    ...
    // First validate that the usr_key field in the user data is a valid ULID
    if let Some(ref usr_key) = user_data.usr_key {
        // Validate ULID format
        if let Err(e) = crate::processors::ulid_generator::validate_ulid(usr_key) {
        ...
        match crate::processors::document_keys::format_user_key(usr_key, &user_data.username) {
            Ok(expected_key) => {
        ...
        let err_msg = "[assert_doc_user] Missing usr_key field in user data".to_string();
        logger!("error", "{}", err_msg);
        return Err(err_msg);
    }
