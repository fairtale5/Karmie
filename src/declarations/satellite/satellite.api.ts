...
import type {_SERVICE as SatelliteActor, ReputationData, Result, Result_1, Result_2} from './satellite.did';
...
export const getUserReputationFull = async (value0: string, value1: string): Promise<Result_1> => {
...
export const testLoggingExposed = async (): Promise<Result_2> => {
	const {test_logging_exposed} = await getSatelliteExtendedActor<SatelliteActor>({
		idlFactory
	});

	return await test_logging_exposed();
}